...
function normalizeEmail(email) {
  ...
  if (!EMAIL_RE.test(e) || e.length > 254) throw new Error('valid email required');
  return e;
}
...
function assertManageable(email) {
  if (env.isEnvManaged(email)) throw new Error('managed by env');
...
async function createUser({ email, displayName, password: pw }) {
  ...
  if (env.isEnvManaged(e)) throw new Error('managed by env');
  if (usersDb.getUser(e)) throw new Error('user exists');
  const hash = await hashValid(pw);
  usersDb.insertUser({ email: e, displayName: cleanName(displayName) });
  usersDb.setCredential(e, hash, true);
...
async function changePassword(email, current, next) {
  ...
  if (next === current) throw new Error('password unchanged');
